...
let earthImg; let earthData; const earthParticles = new THREE.Object3D()
const cloud = new THREE.Object3D()
const hexagon = new THREE.Object3D()
const dotTexture = new THREE.TextureLoader().load('/uploads/user_upload/39231/dot.png')
const coneImg = ['/uploads/user_upload/39231/lightray.jpg', '/uploads/user_upload/39231/lightray_yellow.jpg']
...
function createObjects () {
  // 球面
  const sphereGeom = new THREE.SphereGeometry(CITY_RADIUS, 20, 20)
  const sphereMat = new THREE.MeshBasicMaterial({
    color: 0x03d98e,
    wireframe: true0x03d98e
  })
...
function createHexagon (position, index) {
  ...
  const hexagonPlane = new THREE.CircleGeometry(HEXAGON_RADIUS - CITY_MARGIN, 6)
...
function createCone (position, index) {
  const texture = new THREE.TextureLoader().load(coneImg[index])
  ...
  const geometry = new THREE.PlaneGeometry(HEXAGON_RADIUS * 2, height)
...
gl_FragColor = vec4( texColor.rgb * diffuse, _a );
  ...
  const geometry = new THREE.SphereGeometry(1.3 * radius, 66, 44)
  const map = new THREE.TextureLoader().load('/uploads/user_upload/39231/clouds.jpg')
...
function isLandByUV (c, f) {
  ...
  const n = parseInt(earthImg.width * c) // 根据横纵百分比计算图象坐标系中的坐标
