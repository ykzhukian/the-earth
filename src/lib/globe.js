...
DAT.Globe = function (container, opts) {
  ...
  const mouse = { x: 0, y: 0 }; const mouseOnDown = { x: 0, y: 0 }
  const rotation = { x: 0, y: 0 }
  const target = { x: Math.PI * 3 / 2, y: Math.PI / 6.0 }
  const targetOnDown = { x: 0, y: 0 }

  let distance = 100000; let distanceTarget = 100000
  // const padding = 40
  ...
  function addData (data, opts) {
    ...
    const subgeo = new THREE.BoxGeometry()
    for (i = 0; i < data.length; i += step) {
      lat = data[i]
      lng = data[i + 1]
      color = colorFnWrapper(data, i)
  ...
  function addPoint (lat, lng, size, color, subgeo) {
    ...
    // console.log('point.geometry', point.geometry)
    for (let i = 0; i < point.geometry.groups.length; i++) {
      point.geometry.groups[i].color = color
    }
